...
def _process_coordinates(df, has_end=True):
    ...
    if has_end:
        df['endX'] = df['coord'].apply(lambda c: c['2']['x'] if isinstance(c, dict) and '1' in c else None)
        df['endY'] = df['coord'].apply(lambda c: c['2']['y'] if isinstance(c, dict) and '1' in c else None)
